function(input, output, session) {
  ...
  output$word_total <- renderText({
    ...
    paste0("Shakespeare used the word \"", input$focus_word, "\" ",
           prettyNum(shake_words[word %in% focus_word(), .N], big.mark = ","),
           ifelse(shake_words[word %in% focus_word(), .N] == 1, " time", " times") , " in his plays."
    )
  })
...
output$word_frequency <- renderText({
  ...
  if(length(top_plays()) == 1) {
    paste0("The play that uses the word \"", input$focus_word, "\" most often is ",
                top_plays(), ", which uses it ", 
                prettyNum(word_in_plays()[, focus_word_freq][1], big.mark = ","), 
                ifelse(word_in_plays()[, focus_word_freq][1] == 1, " time.", " times.")
    )
  } else {
    paste0("The plays that use the word \"", input$focus_word, "\" most often are ",
                paste0(top_plays(), collapse = " & "), ", which use it ", 
                prettyNum(word_in_plays()[, focus_word_freq][1], big.mark = ","), 
                ifelse(word_in_plays()[, focus_word_freq][1] == 1, " time.", " times.")
    )
  }
...
output$word_playtype <- renderText({
  req(input$focus_word != "")
  req(focus_word() %in% shake_words$word)
  paste0(focus_type(), " most often use the word \"", input$focus_word, "\", 
         relative to their total number of words.")
})
...
output$random_line <- renderText({
  ...
  random_sentence <- paste0("\"", select_sentence[, sentence], "\" - ", select_sentence[, title])
